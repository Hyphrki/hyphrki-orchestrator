import { I as IterMode, T as Tree, N as NodeProp, P as Parser, a as NodeSet, b as NodeType, D as DefaultBufferLength, c as TreeFragment } from "../@lezer/common-ff1769bd.js";
class Stack {
  /// @internal
...
class TokenGroup {
  ...
  token(input, stack) {
    readToken$1(this.data, input, stack, this.id);
  }
}
...
function readToken$1(data, input, stack, group) {
  let state = 0, groupMask = 1 << group, { parser: parser2 } = stack.p, { dialect: dialect2 } = parser2;
  scan:
    for (; ; ) {
      ...
      for (let i = state + 3; i < accEnd; i += 2)
        if ((data[i + 1] & groupMask) > 0) {
          let term = data[i];
          if (dialect2.allows(term) && (input.token.value == -1 || input.token.value == term || parser2.overrides(term, input.token.value))) {
...
function insertBracket(state, bracket2) {
  ...
  for (let tok of tokens2) {
    ...
    if (bracket2 == tok)
      return closed == tok ? handleSame(state, tok, tokens2.indexOf(tok + tok + tok) > -1, conf) : handleOpen(state, tok, closed, conf.before || defaults$1.before);
...
const LineComment = 1, BlockComment = 2, String$1 = 3, Number = 4, Bool = 5, Null = 6, ParenL = 7, ParenR = 8, BraceL = 9, BraceR = 10, BracketL = 11, BracketR = 12, Semi = 13, Dot = 14, Operator = 15, Punctuation = 16, SpecialVar = 17, Identifier = 18, QuotedIdentifier = 19, Keyword = 20, Type = 21, Bits = 22, Bytes = 23, Builtin = 24, Whitespace = 25;
function isAlpha(ch) {
...
const parser$1 = /* @__PURE__ */ LRParser.deserialize({
  version: 14,
  states: "&`QYQROOO!pQRO'#DUO$dQRO'#CxO&VQRO'#CzO&^QRO'#C{O&eQRO'#C|O&lQRO'#DVOOQQ'#DU'#DUOOQQ'#DQ'#DQO&tQRO'#DOOOQQ'#Cw'#CwOOQQ'#DP'#DPQYQROOQOQQOOO'OQRO'#DRO'^QRO,59dO)QQQO,59dOOQQ,59f,59fO)VQRO,59fOOQQ,59g,59gO)^QRO,59gOOQQ,59h,59hO)eQRO,59hO)lQRO,59qO)tQRO,59qOOQQ-E7O-E7OOOQQ,59c,59cOOQQ-E6}-E6}OOQQ,59m,59mOOQQ-E7P-E7PO)yQRO1G/OOOQQ1G/Q1G/QOOQQ1G/R1G/ROOQQ1G/S1G/SOOQQ1G/]1G/]O+mQRO1G/]O+rQRO1G/]P+zQRO'#DQOOQQ7+$w7+$wO,RQRO7+$wOOQQ<<Hc<<Hc",
  stateData: ",W~OPOSQOS~ORVOSVOTVOUVOVROXSOZTO]YO^QO_VO`VOaPObPOcPOdVOeVOfVOgVOhVOiVOmPOqUO~O^^ORxXSxXTxXUxXVxXXxXZxX]xX_xX`xXaxXbxXcxXdxXexXfxXgxXhxXixXmxXqxXvxXWxXYxX[xX~Oa`Ob`Oc`Om`O~ORVOSVOTVOUVOVROXSOZTO^QO_VO`VOaPObPOcPOdVOeVOfVOgVOhVOiVOmPOqUO~OWaO~P$rOYcO~P$rO[eO~P$rOihOmgO~O]jOvrX~P$rOalOblOclOmlO~O^^ORlaSlaTlaUlaVlaXlaZla]la_la`laalablacladlaelaflaglahlailamlaqlavlaWlaYla[la~O^^O~OWoO~P$rOYpO~P$rO[qO~P$rOisOqrO~OmtO~O^^ORliSliTliUliVliXliZli]li_li`lialibliclidlielifliglihliilimliqlivliWliYli[li~OqvO~OiwOqvO~O]jO~P$rOqxO~O",
  goto: "#czPPPPPPPPPPPPPPPPPPPPPPPPPPP{!PP!P!P!PP!]!a!g!{PP#V!PTZO[eVORSTX[bdfuT]O[Q[ORk[SXO[QbRQdSQfTZiXbdfuQ_PSm_nRn`eWORSTX[bdfu",
  nodeNames: "⚠ LineComment BlockComment String Number Bool Null ( ) { } [ ] ; . Operator Punctuation SpecialVar Identifier QuotedIdentifier Keyword Type Bits Bytes Builtin Whitespace Script Statement CompositeIdentifier Resolvable Parens Braces Brackets OrphanSingleQuote Statement",
  maxTerm: 41,
  skippedNodes: [0, 1, 2],
  repeatNodeCount: 3,
  tokenData: "#o~RQwxX#o#p^~^Oq~~aP#o#pd~gTO#Od#O#Pv#P#qd#q#r#Q#rG|d~yTO#Od#O#Pv#P#qd#q#r!Y#rG|d~!]TO#qd#q#r!l#r;'Sd;'S;=`#i<%lOd~!qTm~O#Od#O#Pv#P#qd#q#r#Q#rG|d~#TTO#qd#q#r#d#r;'Sd;'S;=`#i<%lOd~#iOm~~#lP;=`<%ld",
  tokenizers: [0, tokens],
  topRules: { "Script": [0, 26] },
  tokenPrec: 0
});
...
function getAliases(doc2, at) {
  ...
  for (let scan = statement.firstChild, sawFrom = false, prevID = null; scan; scan = scan.nextSibling) {
    ...
    if (!sawFrom) {
      ...
      while (skippedTokens.includes(next.name))
        next = next.nextSibling;
      if (plainID(next))
        alias = idName(doc2, next);
...
function completeFromSchema(schema, tables, schemas, defaultTableName, defaultSchemaName) {
  ...
  top2.list = defaultSchema.list.concat(schemas || top2.childCompletions("type"));
  return (context) => {
...
function schemaCompletionSource(config2) {
  return config2.schema ? completeFromSchema(config2.schema, config2.tables, config2.schemas, config2.defaultTable, config2.defaultSchema) : () => null;
}
...
export {
  ...
  IndentContext as I,
  ifIn as J,
  completionStatus as K,
  LRParser as L,
  codePointSize as M,
  codePointAt as N,
  insertBracket as O,
  startCompletion as P,
  closeBrackets as Q,
  Decoration as R,
  StateField as S,
  Transaction as T,
  syntaxHighlighting as U,
  HighlightStyle as V,
  ensureSyntaxTree as W,
  bracketMatching as X,
  autocompletion as Y,
  keymap as Z,
  indentOnInput as _,
  syntaxTree as a,
  highlightActiveLineGutter as a0,
  foldGutter as a1,
  dropCursor as a2,
  highlightActiveLine as a3,
  EditorState as a4,
  hoverTooltip as a5,
  showPanel as a6,
  getPanel as a7,
  ...
  gutter as ad,
  showTooltip as ae,
  highlightSpecialChars as af,
  acceptCompletion as ag,
  Compartment as ah,
  Prec as ai,
  StandardSQL as aj,
  PostgreSQL as ak,
  ...
  Cassandra as ap,
  PLSQL as aq,
  keywordCompletionSource as ar,
  LRLanguage as b,
  continuedIndent as c,
  foldInside as d,
  LanguageSupport as e,
  foldNodeProp as f,
  snippetCompletion as g,
  flatIndent as h,
  indentNodeProp as i,
  delimitedIndent as j,
  ifNotIn as k,
  completeFromList as l,
  EditorView as m,
  EditorSelection as n,
  combineConfig as o,
  ChangeSet as p,
  ChangeDesc as q,
  StateEffect as r,
